...
export class ActorSheetBitD extends ActorSheet {
  ...
  getData() {
    ...
    // Ability Scores
    for ( let [a, act] of Object.entries(data.actor.data.actions)) {
      act.icon = this._getProficiencyIcon(abl.proficient);
      act.hover = CONFIG.BITD.proficiencyLevels[abl.proficient];
      act.label = CONFIG.BITD.actions[a];
    }
    ...
    for ( let [s, skl] of Object.entries(data.actor.data.skills)) {
      ...
      skl.hover = CONFIG.BITD.proficiencyLevels[skl.value];
      skl.label = CONFIG.BITD.skills[s];
    }
  ...
  _prepareTraits(traits) {
    const map = {
      "dr": CONFIG.BITD.damageTypes,
      "di": CONFIG.BITD.damageTypes,
      "dv": CONFIG.BITD.damageTypes,
      "ci": CONFIG.BITD.conditionTypes,
      "languages": CONFIG.BITD.languages,
      "armorProf": CONFIG.BITD.armorProficiencies,
      "weaponProf": CONFIG.BITD.weaponProficiencies,
      "toolProf": CONFIG.BITD.toolProficiencies
    };
    ...
    let spellbook = spells.reduce((sb, spell) => {
      ...
      if ( !sb[lvl] ) {
        sb[lvl] = {
          ...
          label: lvl >= 0 ? CONFIG.BITD.spellLevels[lvl] : CONFIG.BITD.spellPreparationModes[mode],
          spells: [],
  ...
  async _onDrop (event) {
    ...
    if (data && (data.type === "Actor")) {
      if (game.user.isGM || (game.settings.get('BITD', 'allowPolymorphing') && this.actor.owner)) {
